...
class ExpBIRCH(Experiments):
    ...
    def perform(clustering_bool: bool,
        ...
        Experiments.run(clustering_bool=clustering_bool,
                        ...
                        algo_str=algo_str,
                        results_folder=os.path.join(os.path.dirname(os.path.dirname(__file__)), BIRCH_RESULTS_FOLDER_NAME),
                        data_path=os.path.join(os.path.dirname(os.path.dirname(__file__)), BIRCH_DATA_FOLDER_NAME),
                        #data_generation_function=BIRCHDataGenerator.generate_kmeans,
                        clustering_algo=BIRCH,
                        numerical_run_times=BIRCH_NUMERICAL_RUN_TIMES,
                        n_clusters=BIRCH_N_CLUSTERS
                        #numerical_generations=BIRCH_NUMERICAL_GENERATION_COUNT,
                        #numerical_population=KEANS_NUMERICAL_POP_SIZE,
                        #analytical_run_times=BIRCH_ANALYTICAL_RUN_TIMES,
                        #analytical_generations=BIRCH_ANALYTICAL_GENERATION_COUNT,
                        #analytical_population=BIRCH_NUMERICAL_POP_SIZE,
                        #parsimony_coefficient=BIRCH_ANALYTICAL_PARSIMONY_COEFFICIENT,
                        #k_fold=K_FOLD,
                        #samples=BIRCH_NUMERICAL_NUM_SAMPLES,
                        #rhoa_range=SFF_RHOA_RANGE,
                        #rhop_range=BIRCH_RHOP_RANGE,
                        #nu_range=BIRCH_NU_RANGE,
                        #re_range=BIRCH_RE_RANGE,
                        #genetic_algorithm_feature_selection.pygenetic_algorithm_feature_selection.pyexpected_eq="unknown",
                        #ebs_size_range=BIRCH_DIMENSIONAL_EBS_SIZE_RANGE_1_2
                        )
